...
--**************************************************************************************************
--	EDITADO POR GUTO, FABIANO-VIKING, [BRA]SON dos esquadrões WOLFPACK e BRA
--	ÚLTIMA MODIFICAÇÃO EM: 04 de Março de 2022
--**************************************************************************************************
-- Original: CSAR Script for DCS Ciribob - 2015 - Version 1.9.2 - 23/04/2018
...
csar.aircraftType["Ka-50"] = 2
csar.aircraftType["UH-1H"] = 12
csar.aircraftType["Mi-24P"] = 8
csar.aircraftType["Mi-8MT"] = 16
csar.aircraftType["Ch47_Chinook"] = 30
csar.aircraftType["UH-60L"] = 14
csar.aircraftType["Hercules"] = 50
...
csar.autosmoke = true -- Automatically Smoke when CSAR helicopter is at 5 km
...
-- 		COMO FUNCIONA O MODO CSAR EM CASO DE PILOTO EJETADO
--      0 - No Limit - NO Aircraft disabling (Aeronaves continuam ativadas sempre. Todas as aeronaves continuam funcionando normalmente mesmo se o piloto ejetar ou for abatido.)
--      1 - Disable Aircraft when its down - Timeout to reenable aircraft (Desativa o slot da aeronave quando ela for abatida. Após um limite de tempo o slot fica disponível.)
--      2 - Disable Aircraft for Pilot when he's shot down -- timeout to reenable pilot for aircraft (Desativa o slot da aeronave abatida somente para a pessoa que estava voando ela. Após um limite de tempo o slot fica disponível.)
...
csar.disableTimeoutTime = 15 -- Time in minutes for TIMEOUT
...
csar.enableForRED = false -- enable for red side --HABILITAR O SCRIPT CSAR PRO LADO RED

csar.enableForBLUE = true -- enable for blue side --HABILITAR O SCRIPT CSAR PRO LADO BLUE
...
csar.messageTime = 40 -- Time to show the intial wounded message for in seconds

csar.weight = 100

csar.pilotRuntoExtractPoint = true -- Downed Pilot will run to the rescue helicopter up to csar.extractDistance METERS 

csar.loadDistance = 40 -- configure distance for pilot to get in helicopter in meters.
...
function csar.resetAllPilotLives()
    ...
    csar.pilotLives = {}
    env.info("Vidas dos pilotos resetadas!")
end
...
function csar.addCsar(_coalition , _country, _point, _typeName, _unitName, _playerName, _freq, noMessage, _description )
  ...
  if noMessage ~= true then
    trigger.action.outTextForCoalition(_spawnedGroup:getCoalition(), "Mayday! Mayday! " .. _typeName .. " caindo. ", 10)
	trigger.action.outSound('MAYDAY.ogg') -- Toca o áudio de Mayday junto a mensagem acima para todos quando uma aeronave estiver caindo. 
  end
...
-- Generate DESCRIPTION text
  ...
  if _playerName ~= nil then
      _text = "Piloto " .. _playerName .. " no " .. _typeName
	  --_text = "Piloto " .. _playerName .. " do " .. _unitName .. " - " .. _typeName  --ESSA ERA A LINHA ORIGINAL
  elseif _typeName ~= nil then
      _text = "Piloto AI - " .. _typeName
	  --_text = "Piloto AI do " .. _unitName .. " - " .. _typeName -- LINHA ORIGINAL
  else
      _text = _description
  end
...
function csar.spawnCsarAtZone( _zone, _coalition, _description, _randomPoint)
  ...
  if _triggerZone == nil then
    trigger.action.outText("Csar.lua ERRO: Não foi possível encontrar a zona chamada " .. _zone, 10)
    return
  end
    ...
    local status, err = pcall(function(_event)
        ...
        elseif (_event.id == 9 and csar.csarOncrash == false) then
            ...
            if csar.takenOff[_event.initiator:getName()] == true or _unit:inAir() then
                ...
                trigger.action.outTextForCoalition(_unit:getCoalition(), "Mayday! Mayday! " .. _unit:getTypeName() .. " Abatido. Sem paraquedas!", 10)
            ...
            if _details ~= nil then
                ...
                if _details.noPilot then
                    ...
                    if csar.disableAircraftTimeout then
                        ...
                        local _text = string.format("Essa aeronave não pode voar porque o piloto foi abatido. Reinforcements in %.2dM,%.2dS\n\nSerá destruída ao decolar!", (_time / 60), _time % 60)

                        --display message,
                        csar.displayMessageToSAR(_unit, _text, 10, true)
                    else
                        --display message,
                        csar.displayMessageToSAR(_unit, "Essa aeronave não pode voar porque o piloto foi abatido\n\nSerá destruída ao decolar!", 10, true)
                    end
                else
                    if csar.disableAircraftTimeout then
                        --display message,
...
function csar.createUnit(_x, _y, _heading, _type)
    ...
    local _name = string.format("Piloto ferido #%s", _id)
...
function csar.initSARForPilot(_downedGroup, _freq)
    ...
    local
    _text = string.format("%s solicita resgate na posição %s, frequência beacon %.2f KHz",
...
function csar.checkWoundedGroupStatus(_argument)
    local _status, _err = pcall(function(_args)
        ...
        if csar.checkGroupNotKIA(_woundedGroup, _woundedGroupName, _heliUnit, _heliName) then
            ...
            if _distance < 2500 then --DISTANCIA QUE OS PILOTOS A SEREM RESGATADOS CONSEGUIREM AVISTAR OS HELOS. NÚMERO ORIGINAL ERA 3000 (3km)
...
function csar.pickupUnit(_heliUnit, _pilotName, _woundedGroup, _woundedGroupName)
    ...
    csar.displayMessageToSAR(_heliUnit, string.format("%s: %s Embarquei! Leve-me para uma base ou Farp mais próxima! ", _heliName, _pilotName), 10)
	trigger.action.outSound('CS - Ok Lets Go.ogg')
...
-- Helicopter is within 3km / HELICÓPTERO ESTÁ DENTRO DE UM RAIO DE 3 KM
function csar.checkCloseWoundedGroup(_distance, _heliUnit, _heliName, _woundedGroup, _woundedGroupName)
    ...
    if csar.heliVisibleMessage[_lookupKeyHeli] == nil then
        if csar.autosmoke == true then
          csar.displayMessageToSAR(_heliUnit, string.format("%s: %s. Já consigo ouvir seu helicóptero! Pouse perto da fumaça ou faça um hover para me resgatar.", _heliName, _pilotName), 30)
  		else
          csar.displayMessageToSAR(_heliUnit, string.format("%s: %s. Já consigo ouvir seu helicóptero! Solicite um Flare (sinalizador) ou Fumaça se você precisar.", _heliName, _pilotName), 30)
        end
        --mark as shown for THIS heli and THIS group
        csar.heliVisibleMessage[_lookupKeyHeli] = true
    end

    if (_distance < 800) then --DISTANCIA QUE O PILOTO ABATIDO AVISTA OS HELOS E MANDA A SEGUNDA MENSAGEM (numero original era 500)

        if csar.heliCloseMessage[_lookupKeyHeli] == nil then
            if csar.autosmoke == true then
              csar.displayMessageToSAR(_heliUnit, string.format("%s: %s. Estou aqui em baixo! Pouse ou faça um hover perto da fumaça.", _heliName, _pilotName), 10)
			  trigger.action.outSound('Down Here.ogg') -- Toca o áudio junto a mensagem acima
            else
              csar.displayMessageToSAR(_heliUnit, string.format("%s: %s. Estou aqui em baixo! Pouse em um local seguro, eu me deslocarei. ", _heliName, _pilotName), 10)
            end
            --mark as shown for THIS heli and THIS group
          ...
          if csar.pilotRuntoExtractPoint == true then
              if (_distance < csar.extractDistance) then
                ...
                if _time == nil then
                    ...
                    csar.displayMessageToSAR(_heliUnit, "Espere até " .. _pilotName .. ". embarque em \n" .. _time .. " segundos.", 10, true)
                else
            ...
            if csar.inAir(_heliUnit) and _unitsInHelicopter + 1 <= _maxUnits then
                ...
                if _distance < 25.0 then  --DISTANCIA ORIGINAL ERA DE 8.0 --DISTÂNCIA MÍNIMA PARA RESGATE DE PILOTO ABATIDO CSAR

                    --check height!
                    local _height = _heliUnit:getPoint().y - _woundedLeader:getPoint().y

                    if _height <= 100.0 then --ALTURA ORIGINAL ERA DE 20.0 --ALTURA MÍNIMA PARA RESGATE DE PILOTO ABATIDO CSAR
                        ...
                        if _time > 0 then
                            csar.displayMessageToSAR(_heliUnit, "Fazendo um hover acima de " .. _pilotName .. ". \n\nMantenha o hover por " .. _time .. " segundos para içá-los. \n\nSe o temporizador parar você está muito longe!", 10, true)
                        else
                        ...
                        csar.displayMessageToSAR(_heliUnit, "Alto demais para içar " .. _pilotName .. " \nReduza altura e faça um hover por 10 segundos!", 5, true)
                    end
                end
...
function csar.rescuePilots(_heliUnit)
    ...
    local _txt = string.format("%s: Os pilotos resgatados foram levamos para o \nalojamento ou hospital em segurança. Bom trabalho!", _heliUnit:getName())
	trigger.action.outSound('Soldiers Rescued 2 voz limpa.ogg') -- Toca o áudio junto a mensagem acima
	-- enable pilots again
    for _, _rescueGroup in pairs(_rescuedGroups) do
...
function csar.displayActiveSAR(_unitName)
    local _msg = "PILOTOS ABATIDOS PARA RESGATAR:"
...
function csar.signalFlare(_unitName)
    ...
    if _closet ~= nil and _closet.pilot ~= nil and _closet.distance < 8000.0 then
        ...
        local _msg = string.format("%s - %.2f KHz ADF - %.3fM - Lançando sinais de flare às suas %s ", _closet.groupInfo.desc, _closet.groupInfo.frequency / 1000, _closet.distance, _clockDir)
        csar.displayMessageToSAR(_heli, _msg, 20)

        trigger.action.signalFlare(_closet.pilot:getPoint(), 1, 0)
    else
        csar.displayMessageToSAR(_heli, "Não há pilotos em um raio de 8KM", 20)
    end
end
...
function csar.reqsmoke( _unitName )
    ...
    if _closet ~= nil and _closet.pilot ~= nil and _closet.distance < 8000.0 then
        ...
        local _msg = string.format("%s - %.2f KHz ADF - %.3fM - Lançando fumaça às suas %s ", _closet.groupInfo.desc, _closet.groupInfo.frequency / 1000, _closet.distance, _clockDir)
        csar.displayMessageToSAR(_heli, _msg, 20)
...
function csar.checkOnboard(_unitName)
    ...
    if _inTransit == nil or csar.tableLength(_inTransit) == 0 then
        csar.displayMessageToSAR(_unit, "Não há pilotos resgatados a bordo", 30)
    else

        local _text = "Pilotos abatidos a bordo: "
        ...
        if _unit ~= nil then
            ...
            if _groupId then
                ...
                if csar.addedTo[tostring(_groupId)] == nil then
                    ...
                    local _rootPath = missionCommands.addSubMenuForGroup(_groupId, "Resgates (CSAR)")

                    missionCommands.addCommandForGroup(_groupId, "Listar Solicitações de resgate ativos", _rootPath, csar.displayActiveSAR,
                        _unitName)

                    missionCommands.addCommandForGroup(_groupId, "Checar a Bordo", _rootPath, csar.checkOnboard, _unitName)

                    missionCommands.addCommandForGroup(_groupId, "Solicitar Sinalizador Flare", _rootPath, csar.signalFlare, _unitName)
                    missionCommands.addCommandForGroup(_groupId, "Solicitar Fumaça", _rootPath, csar.reqsmoke, _unitName)
