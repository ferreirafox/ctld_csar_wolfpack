...
function JTACAutoLase(jtacGroupName, laserCode,smoke,lock,colour)
    ...
    if jtacGroup == nil or #jtacGroup == 0 then
        ...
        notify("Grupo JTAC " .. jtacGroupName .. " destruído!", 10)
  ...
  --      env.info("TEMP UNIT INFO: " .. tempUnitInfo.name .. " " .. tempUnitInfo.unitType)
        ...
        if tempUnit ~= nil and tempUnit:getLife() > 0 and tempUnit:isActive() == true then
            notify(jtacGroupName .. " alvo " .. tempUnitInfo.unitType .. " perdido. Buscando mais alvos. ", 10)
        else
            notify(jtacGroupName .. " alvo " .. tempUnitInfo.unitType .. " destruído. Bom trabalho! Buscando mais alvos. ", 10)
        end
    ...
    if enemyUnit == nil then
        ...
        if enemyUnit ~= nil then
            ...
            notify(jtacGroupName .. " iluminando com laser novo alvo " .. enemyUnit:getTypeName() .. '. CODE: ' .. laserCode ..getPositionString(enemyUnit) , 10)
    ...
    for jtacGroupName, jtacUnitName in pairs(GLOBAL_JTAC_UNITS) do
        ...
        if jtacUnit ~= nil and jtacUnit:getLife() > 0 and jtacUnit:isActive() == true then
            ...
            if enemyUnit ~= nil and enemyUnit:getLife() > 0 and enemyUnit:isActive() == true then
                message = message .. "" .. jtacGroupName .. " travando " .. enemyUnit:getTypeName().. " CÓDIGO: ".. laserCode .. getPositionString(enemyUnit) .. "\n"
            else
                message = message .. "" .. jtacGroupName .. " buscando por alvos" .. getPositionString(jtacUnit) .."\n"
            end
        end
    end
    ...
    if blueGroups ~= nil then
        for x, tmpGroup in pairs(blueGroups) do
            ...
            if GLOBAL_JTAC_RADIO_ADDED[index] == nil then
                ...
                missionCommands.addCommandForGroup(Group.getID(tmpGroup), "Status do JTAC", nil, getJTACStatus, nil)
